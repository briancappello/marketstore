...
def test_write_not_allowed_on_replica():
    ...
    # --- query and assert a "no results" error is returned ---
    with pytest.raises(Exception) as excinfo:
        # frontend/query.go wraps the parser's "no files returned from query
        # parse" into "no results returned from query: Target: ...".
        replica_client.query(pymkts.Params('REPL', '1Min', 'OHLCV'))
    assert "no results returned from query" in str(excinfo.value)
